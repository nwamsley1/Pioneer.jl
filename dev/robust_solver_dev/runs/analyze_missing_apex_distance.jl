...
const RUN16     = "/Users/nathanwamsley/Data/RegressionTestsLite/MTAC_3P_Standard/sweep_l1robust/16_combined"
...
pl16 = DataFrame(Arrow.Table(joinpath(RUN16,"precursors_long.arrow")))
qcol = hasproperty(pl16, :MBR_boosted_qval) ? :MBR_boosted_qval : :qval
pl16 = pl16[(pl16[!, qcol] .< 0.01) .& pl16.target, :]
pass16_by_file = Dict{String, Set{UInt32}}()
for sub in groupby(pl16, :file_name)
...
for fn in FILES
    ...
    P16 = pass16_by_file[fn]                    # passing in 16
    miss_pids = setdiff(Set(keys(H)), P16)
    ...
    f0 = DataFrame(Tables.columntable(Arrow.Table(joinpath(RUN16,"temp_data","main_search_psms","$(fn)_fold0.arrow"))))
    f1 = DataFrame(Tables.columntable(Arrow.Table(joinpath(RUN16,"temp_data","main_search_psms","$(fn)_fold1.arrow"))))
