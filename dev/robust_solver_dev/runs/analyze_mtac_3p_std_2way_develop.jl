# 2-way comparison on MTAC_3P_Standard:
#   develop_baseline — develop @ 932b8c77 (q_prescore=0.10 hardcoded, MBR on)
#   hupo2026         — external reference: HUPO2026/MtacThreeProteomeStandard_hupo2026_v0o6o6
#
# Outputs in results/mtac_3p_std_2way_develop_vs_hupo/.
...
const OUT = joinpath(@__DIR__, "..", "results", "mtac_3p_std_2way_develop_vs_hupo")
...
const COLORS = Dict("develop_baseline" => :dodgerblue, "hupo2026" => :firebrick)
...
for (n, p) in CELLS
    ...
    results[n] = (df = df, fc = fc, cv6 = cv6, mp = mp, h = h,
                  np = length(unique(df.precursor_idx)))
    println("[loaded] ", n, " rows=", nrow(df), " unique_prec=", results[n].np,
            " HUMAN-in-all-6=", length(cv6))
end

# ── Summary TSV ──
open(joinpath(OUT, "summary.tsv"), "w") do io
    println(io, "cell\tn_prec_unique\tspecies\tn_paired\tlog2fc_med\tlog2fc_mad\toutlier_|>2|\toutlier_|>4|")
    for (n, _) in CELLS
        haskey(results, n) || continue
        s = results[n]
        for sp in unique(s.fc.species)
            sub = s.fc[s.fc.species .== sp, :]
            isempty(sub) && continue
            println(io, join(Any[n, s.np, sp, nrow(sub),
                @sprintf("%.4f", median(sub.log2fc)),
                @sprintf("%.4f", mad(sub.log2fc, normalize = false)),
                @sprintf("%.4f", mean(abs.(sub.log2fc) .> 2)),
                @sprintf("%.4f", mean(abs.(sub.log2fc) .> 4)),
            ], '\t'))
        end
    end
end
...
# ── HUMAN multiplicity ──
open(joinpath(OUT, "human_multiplicity.tsv"), "w") do io
    println(io, "cell\tN=6\tN=5\tN=4\tN=3\tN=2\tN=1\ttotal_unique_human")
    for (n, _) in CELLS
        haskey(results, n) || continue
        h = results[n].h
        cs = [count(==(k), h.n_files) for k in 1:6]
        println(io, join(Any[n, cs[6], cs[5], cs[4], cs[3], cs[2], cs[1], nrow(h)], '\t'))
    end
end

# ── HUMAN-in-all-6 by CV cutoff ──
open(joinpath(OUT, "human_cv_in_6.tsv"), "w") do io
    print(io, "cell\tHUMAN_in_all_6")
    for cv in CV_BINS
        print(io, "\tCV≤", round(cv, digits=2))
    end
    println(io)
    for (n, _) in CELLS
        haskey(results, n) || continue
        ...
        println(io, join(Any[n, length(cv6), cs...], '\t'))
    end
end
...
for sp in species_list
    plt = plot(size = (900, 500),
               title = "MTAC_3P_Standard log2(A/B) — $sp",
    ...
    for (n, _) in CELLS
        ...
        isempty(sub) && continue
        stephist!(plt, sub.log2fc, bins = -6:0.1:6, label = n,
                  color = COLORS[n], linewidth = 2, normalize = :pdf)
    end
    savefig(plt, joinpath(OUT, "plots", "log2fc_$(sp).png"))
end

plt_cv = plot(size = (1000, 500),
              title = "HUMAN-in-all-6: cumulative count by CV (across all 6 reps)",
...
for (n, _) in CELLS
    ...
    plot!(plt_cv, CV_BINS, ys, marker = :circle, linewidth = 2,
