...
const RUN18     = "/Users/nathanwamsley/Data/RegressionTestsLite/MTAC_3P_Standard/sweep_l1robust/18_consensus_irt"
...
pl18 = DataFrame(Arrow.Table(joinpath(RUN18,"precursors_long.arrow")))
qcol = hasproperty(pl18, :MBR_boosted_qval) ? :MBR_boosted_qval : :qval
pl18 = pl18[(pl18[!, qcol] .< 0.01) .& pl18.target, :]
pass18_by_file = Dict{String, Set{UInt32}}()
for sub in groupby(pl18, :file_name)
...
for fn in FILES
    ...
    P18 = pass18_by_file[fn]                    # passing in 16
    miss_pids = setdiff(Set(keys(H)), P18)
    ...
    f0 = DataFrame(Tables.columntable(Arrow.Table(joinpath(RUN18,"temp_data","main_search_psms","$(fn)_fold0.arrow"))))
    f1 = DataFrame(Tables.columntable(Arrow.Table(joinpath(RUN18,"temp_data","main_search_psms","$(fn)_fold1.arrow"))))
