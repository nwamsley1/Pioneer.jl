...
|---|---|---|---:|---|---|
| A1 | `n_scans_left_capped3` | `min(k-1, 3)` | TBD | ⏳ | |
| A2 | `n_scans_right_capped3` | `min(N-k, 3)` | TBD | ⏳ | |
| A3 | `min_edge_distance` | `min(A1, A2)` | TBD | ⏳ | |
| A4 | `is_at_edge` | `A1==0 \|\| A2==0` (bool) | TBD | ⏳ | |
| A5 | `relative_position` | `(k-1) / max(N-1, 1)` | TBD | ⏳ | |
| A6 | `dist_to_relative_center` | `\|0.5 - A5\|` | TBD | ⏳ | |
...
|---|---|---|---:|---|---|
| B1 | `weight_apex_relative_pos` | `argmax(w_chrom) / max(N-1, 1)` | TBD | ⏳ | |
| B2 | `weight_chrom_skewness` | std skewness `E[((x-μ)/σ)^3]` of weight | TBD | ⏳ | |
| B3 | `weight_chrom_kurtosis` | excess kurtosis `E[((x-μ)/σ)^4] - 3` | TBD | ⏳ | |
| B4 | `apex_to_edge_weight_ratio` | `max(w) / max(w_first, w_last)` | TBD | ⏳ | |
| B5 | `n_above_hm_left_of_apex` | scans left of apex with w > 0.5·max(w) | TBD | ⏳ | |
| B6 | `n_above_hm_right_of_apex` | symmetric | TBD | ⏳ | |
| B7 | `hm_asymmetry` | `\|B5 - B6\| / max(B5+B6, 1)` | TBD | ⏳ | |
...
| # | Feature | Definition | Gain | Status | Notes |
|---|---|---|---:|---|---|
| C1 | `weight_chrom_gaussian_r2` | R² of log-quadratic fit on weight chrom | TBD | ⏳ | |
| C2 | `weight_chrom_gaussian_sigma` | fitted σ (peak width); requires c<0 | TBD | ⏳ | |
| C3 | `weight_chrom_gaussian_apex_irt` | fitted apex iRT (peak center) | TBD | ⏳ | |
...
| # | Feature | Definition | Gain | Status | Notes |
|---|---|---|---:|---|---|
| D1 | `shape_m2` | normalized weight at scan-position k-2 (vs current PSM scan) | TBD | ⏳ | |
| D2 | `shape_m1` | normalized weight at k-1 | TBD | ⏳ | |
| D3 | `shape_0` | normalized weight at k | TBD | ⏳ | |
| D4 | `shape_p1` | normalized weight at k+1 | TBD | ⏳ | |
| D5 | `shape_p2` | normalized weight at k+2 | TBD | ⏳ | |
| D6 | `gof_minus_max_gof_precursor` | `gof - max(gof over precursor's scans)` (≤0) | TBD | ⏳ | |
| D7 | `gof_fraction_of_total_precursor` | `log(gof / (sum(gof) + 1))` | TBD | ⏳ | |
