...
\For{$(p, a) \in \mathcal{E}^{(r)}_{Y,e}$}
    ...
    \State $G_0[p] \gets \{a : (p, a) \in \mathcal{E}^{(r)}_{Y,e}\}$ \Comment{Store original protein group}
\EndFor
\State
...
\For{$(P_c, A_c) \in C$}
    ...
    \If{$P[a] = P[a']$ for all $a, a' \in A_c$}
        \For{$p \in P_c$}
            \State $G[p] \gets A_c$ \Comment{All peptides unique to protein group (Lines 228-243)}
        \EndFor
    ...
    \While{$R \neq \emptyset$ and $K \neq \emptyset$}
        ...
        \For{$a \in K$}
            \State $R_a \gets P[a] \cap R$ \Comment{Remaining peptides for protein $a$}
            \State $h_a \gets \text{hash}(\text{sort}(R_a))$ \Comment{Hash of peptide set}
        \EndFor
        \State \textit{// Merge proteins with identical remaining peptide sets}
        \State $K' \gets \emptyset$ \Comment{Merged candidate set}
        \For{each unique hash $h$}
            \State $M_h \gets \{a \in K : h_a = h\}$ \Comment{Proteins with same remaining peptides}
            \If{$|M_h| = 1$}
                \State $K' \gets K' \cup M_h$ \Comment{Single protein, no merge}
            \Else
                \State $a_{\text{merged}} \gets \text{join}(\text{sort}(M_h), ";")$ \Comment{Create merged protein (lines 352-357)}
                \State $P[a_{\text{merged}}] \gets P[a]$ for any $a \in M_h$ \Comment{Use remaining peptides (line 362)}
                \State $K' \gets K' \cup \{a_{\text{merged}}\}$
                \For{$a \in M_h$}
                    \State Delete $P[a]$ \Comment{Remove individual entries (lines 365-367)}
                \EndFor
            \EndIf
        \EndFor
        \State $K \gets K'$ \Comment{Update candidates with merged proteins}
        \State
        \State \textit{// Step 2: Find protein/group with best coverage (lines 373-383)}
        \State $a^* \gets \arg\max_{a \in K} |P[a] \cap R|$ \Comment{Protein covering most peptides}
        \If{$|P[a^*] \cap R| = 0$}
            \State \textbf{break} \Comment{Line 386}
        \EndIf
        \State
        \State \textit{// Step 3-4: Add to necessary set and remove covered peptides (lines 389-398)}
        \State $S \gets S \cup \{a^*\}$ \Comment{Line 390}
        \State $R \gets R \setminus P[a^*]$ \Comment{Lines 395-397}
        \State $K \gets K \setminus \{a^*\}$ \Comment{Line 391}
    \EndWhile
    \State
    ...
    \For{$p \in P_c$}
        ...
        \If{$|S_p| = 1$}
            \State $G[p] \gets S_p$ \Comment{Unique assignment - add to result (lines 366-370)}
        \EndIf
        \State \textit{// Note: Shared peptides ($|S_p| > 1$) are excluded from G}
    \EndFor
\EndFor
\State
\State \textbf{return} $G$ \Comment{Line 380}
\end{algorithmic}
\end{algorithm}
