...
- §1.6.2 Cross-Run Precursor Aggregation (was First Pass PSM Aggregation): REWRITTEN. Dropped s_max/M/top-M eqns + second-pass framing. Now: pool all PEP-passing precursors (no top-M); cross-run global score = log-odds avg of top-sqrt(R) raw CV scores (_logodds_combine, prescore_aggregation.jl:159-167); global qvals used ONLY to select high-confidence precursors for RT-tol fit (line 171/111), NOT a filter; full PEP-passing set CV-fold-split carried forward. Verified develop. DONE.
- TODO §1.6.3 RT Alignment+Tolerance: still old (prob>0.95, cross-run iRT variance, FWHM+iRT_var tol). Verify develop: recalibrate_rt! prob>0.9, peptide-composition iRT correction (irt_refinement.jl), RT-binned FWHM/cycle tol.
