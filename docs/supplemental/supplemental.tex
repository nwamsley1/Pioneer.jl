...
In the main search, Pioneer identifies, scores, and filters the precursors carried forward for quantification. For each MS2 scan, the fragment index (Section~\ref{subsec:fragment-index-search}) yields a set of candidate precursors, which are quantified against the spectrum by spectral deconvolution (Section~\ref{subsec:matrix-representation}) and scored by a machine-learned classifier. Pioneer then selects a single best PSM per precursor, removes low-confidence identifications, and aggregates the per-run results into the set of precursors used downstream.

\subsubsection{PSM Scoring}\label{subsubsec:first-pass-search-scoring}

For each MS data file, Pioneer scores its PSMs with a gradient-boosted decision-tree classifier (LightGBM) trained to separate target from decoy PSMs. Training uses two-fold cross-validation: precursors are split into two folds, a model is trained on each fold and used to score the PSMs of the other, so no PSM is scored by a model trained on it. The classifier draws on a large set of features, including spectral goodness-of-fit and similarity scores, fragment-ion coverage, the deconvolution weight and its rank among co-eluting precursors, retention-time agreement, MS1 isotope-pattern features, per-rank fragment chromatographic traces and fragment--fragment correlations, and precursor and sequence properties. For files with too few PSMs to train a stable tree model, Pioneer falls back to a probit regression.

\textbf{Best PSM selection.} Pioneer then selects one PSM per precursor: among a precursor's top-scoring scans, it keeps the one with the largest deconvolution weight, falling back to the highest-scoring scan when only a few PSMs are available.

\textbf{PEP-based filtering.} Pioneer estimates each PSM's posterior error probability (PEP) from the classifier scores by isotonic regression, using the weighted Pool Adjacent Violators Algorithm (wPAVA) \cite{Zheng_pyIsoPEP}. PSMs with $\text{PEP} \leq 0.9$ are retained for each file and the rest are filtered out.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Method - Aggregate First Pass Search
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{First Pass PSM Aggregation}\label{subsubsec:first-pass-psm-aggregation}
After scoring and filtering the run-specific PSM lists, Pioneer assembles a single list of $M$ precursors to include in the second, quantitative search across all MS data files. Let $\mathcal{R} = \{1, \ldots, R\}$ be the set of all runs, and $\mathcal{P}$ be the set of all precursors in the spectral library. Let $P^{r} = \{(p_j^{r}, s_j^{r}, t_j^{r})\}_{j=1}^{N_r}$ be the set of precursors and their respective scores in each run $r \in \mathcal{R}$. Here:
\begin{itemize}
