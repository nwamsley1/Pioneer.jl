...
function splineQuadTransmissionModel(
    ...
    _first = min(minimum(x0), minimum(x1))
    _last = max(maximum(x0), maximum(x1)) 
    bin_width = (_last - _first)/(n_knots - 1)
    knots = collect(LinRange(_first, _last, n_knots))
    X0 = buildDesignMat(x0, collect(knots), bin_width, spline_basis)
    X1 = buildDesignMat(x1, collect(knots), bin_width, spline_basis)
    X = X0 .- X1
    c = X\yt
