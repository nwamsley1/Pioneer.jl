...
"""
    dropVectorColumns!(df::AbstractDataFrame)

Remove all columns whose element type is a subtype of AbstractVector.
Arrow cannot serialize nested vector columns, so this is called before writing.
"""
...
"""
    initialize_prob_group_features!(psms::AbstractDataFrame)

Add zero-initialized `trace_prob` and `q_value` columns to a PSM DataFrame.
"""
