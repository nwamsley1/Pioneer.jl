...
Mutable scoring columns (trace_prob, q_value) live in a separate
"sidecar" file so scoring iterations only rewrite ~12 bytes/PSM instead
...
File layout after prepare_training_data!:
  ...
  {base}_fold0_scores.arrow   ← trace_prob, q_value columns (mutable)
=#
