...
"""
function updateMu!(Hs::AbstractSparseDesignMatrix{Ti, T}, μ::Vector{T}, col::Int64, X1, X0) where {Ti<:Integer, T<:AbstractFloat}
...
"""
function initObserved!(y::Vector{T}, sa::AbstractSparseDesignMatrix{Ti,T}) where {Ti<:Integer, T<:AbstractFloat}
    if length(y) < sa.m
...
"""
function initMu!(μ::Vector{T}, sa::AbstractSparseDesignMatrix{Ti,T}, w::Vector{T}) where {Ti<:Integer, T<:AbstractFloat}
    if length(μ) < sa.m
...
function solvePoissonMM_fast!(Hs::AbstractSparseDesignMatrix{Ti, T},
    ...
    max_weight = T(0)
    # μ floor stays in the storage type (Float32) for clamping
    ε_mu = T(POISSON_MU_FLOOR)
    # L1/L2 threshold in Float64 — much tighter than Float32's 1e-6,
    # avoids the stuck-weight bug where tiny a_ij² products underflow
    ε_L = 1e-15
