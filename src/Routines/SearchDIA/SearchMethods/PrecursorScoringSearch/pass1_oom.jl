...
function _count_psm_rows_by_fold(file_paths::Vector{String}, training_masks = nothing)
    ...
    for (file_idx, fpath) in enumerate(file_paths)
        ...
        mask !== nothing && length(mask) != n &&
            throw(ArgumentError("training mask length for $fpath must match row count"))
        fold_c = tbl.cv_fold
...
function _predict_pass1_to_sidecar(
    ...
    if n == 0
        ...
        return (scores = Float32[], targets = Bool[])
    end
    ...
    @inline function _predict_block(cls, rows::Vector{Int})
        ...
        if cls isa NamedTuple && cls.kind == :constant
            return fill(Float32(cls.value), length(rows))
        end
    ...
    return (scores = oof, targets = Vector{Bool}(tbl.target))
end
...
function _predict_pass1_files(
    ...
    results = Vector{result_type}(undef, length(file_paths))
    parallel_foreach!(length(file_paths)) do chunk
        for f_idx in chunk
            results[f_idx] = _predict_pass1_to_sidecar(
                file_paths[f_idx],
                features,
                cls_trained_on,
                compute_infold,
...
function _collect_pass1_predictions(prediction_results)
    n = sum(length(result.scores) for result in prediction_results)
    scores = Vector{Float32}(undef, n)
    targets = Vector{Bool}(undef, n)
    offset = 0
    for result in prediction_results
        len = length(result.scores)
        if len > 0
            range = (offset + 1):(offset + len)
            scores[range] .= result.scores
            targets[range] .= result.targets
        end
        offset += len
    end
...
function train_and_predict_pass1_oom!(
    ...
    function _run_iteration(iter_idx::Int, training_masks)
        ...
        prediction_results = _predict_pass1_files(
            file_paths,
            available,
            cls_trained_on,
            false;
        ...
        scores, targets = _collect_pass1_predictions(prediction_results)
        metrics = _scoring_score_metrics(
            scores,
            targets;
