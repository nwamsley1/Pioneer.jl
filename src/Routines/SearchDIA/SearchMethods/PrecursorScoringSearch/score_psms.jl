...
function _score_precursor_isotope_traces_mbr(
    ...
    # 1. pid → ranked counterfactual partner candidates (streaming over Arrow tables).
    cf_partner_candidates = build_counterfactual_partner_candidate_matrix(file_paths, precursors)
    ...
    annotation_keys = _mbr_fragment_annotation_key_table(fragment_lookup, length(precursors))

    # Parallelize the per-file MBR feature compute + sidecar write across
    # files. donor_dict and cf_partner_candidates are read-only across this loop
    ...
    cf_partner_candidates = zeros(UInt32, 0, 0)
    annotation_keys = NTuple{8, UInt16}[]
    GC.gc()
