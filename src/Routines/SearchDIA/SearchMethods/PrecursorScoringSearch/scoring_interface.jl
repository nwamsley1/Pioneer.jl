...
function _aggregate_trace_to_precursor_probs!(df::DataFrame)
    ...
    propagate_mainsearch_integration_intervals!(df)
end

function propagate_mainsearch_integration_intervals!(df::DataFrame)
    required_cols = (
        :mainsearch_1pct_start_scan,
        :mainsearch_1pct_stop_scan,
        :mainsearch_1pct_start_rt,
        :mainsearch_1pct_stop_rt,
    )
    all(col -> hasproperty(df, col), required_cols) || return df

    n = nrow(df)
    n == 0 && return df

    prec_ids = df[!, :precursor_idx]::AbstractVector{<:Integer}
    file_ids = hasproperty(df, :ms_file_idx) ? df[!, :ms_file_idx] : fill(UInt32(0), n)
    start_scan = df[!, :mainsearch_1pct_start_scan]::AbstractVector{<:Integer}
    stop_scan = df[!, :mainsearch_1pct_stop_scan]::AbstractVector{<:Integer}
    start_rt = df[!, :mainsearch_1pct_start_rt]::AbstractVector{<:AbstractFloat}
    stop_rt = df[!, :mainsearch_1pct_stop_rt]::AbstractVector{<:AbstractFloat}

    perm = sortperm(1:n, by = i -> (UInt32(prec_ids[i]), UInt32(file_ids[i])))
    group_start = 1
    while group_start <= n
        first_row = perm[group_start]
        pid = UInt32(prec_ids[first_row])
        file_id = UInt32(file_ids[first_row])
        group_stop = group_start
        while group_stop < n
            next_row = perm[group_stop + 1]
            if UInt32(prec_ids[next_row]) != pid || UInt32(file_ids[next_row]) != file_id
                break
            end
            group_stop += 1
        end

        group_start_scan = typemax(UInt32)
        group_stop_scan = UInt32(0)
        group_start_rt = typemax(Float32)
        group_stop_rt = typemin(Float32)
        has_scan_interval = false
        has_rt_interval = false

        @inbounds for pos in group_start:group_stop
            row = perm[pos]
            if start_scan[row] > 0 && stop_scan[row] > 0
                has_scan_interval = true
                group_start_scan = min(group_start_scan, UInt32(start_scan[row]))
                group_stop_scan = max(group_stop_scan, UInt32(stop_scan[row]))
            end
            if !isnan(start_rt[row]) && !isnan(stop_rt[row])
                has_rt_interval = true
                group_start_rt = min(group_start_rt, Float32(start_rt[row]))
                group_stop_rt = max(group_stop_rt, Float32(stop_rt[row]))
            end
        end

        @inbounds for pos in group_start:group_stop
            row = perm[pos]
            if has_scan_interval
