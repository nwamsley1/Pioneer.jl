...
# second-pass Arrow tables, stores shared same-charge m/z pools
...
# the nearest non-self same-charge precursor by m/z that is inside the
# receiver file's calibrated library-iRT tolerance and has a cross-file donor.
# Nothing here
...
function _collect_unique_precursors_streaming(
    ...
    return (pids = plist_pids, mz = plist_mz, charge = plist_charge, irt = plist_irt)
end
...
"""
    ...
    same-charge m/z pools needed for deterministic, file-aware counterfactual
    partner resolution.

    For each receiver row, MBR streaming chooses the nearest non-self precursor
    by library m/z that has the same charge, falls inside the receiver file's
    calibrated iRT tolerance, and has a donor in a file other than the receiver
    file.
"""
