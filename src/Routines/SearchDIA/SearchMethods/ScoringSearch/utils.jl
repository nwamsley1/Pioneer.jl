...
Create and score protein groups from passing PSMs using efficient OOM implementation.
Integrates ML scoring into protein group creation to reduce file I/O.
...
- `max_psms_in_memory`: Maximum PSMs to hold in memory (default: 10000000)

# Returns
- `protein_inference_dict`: Dictionary mapping peptides to protein groups

# Process
1. Count all possible peptides for each protein in library
2. Perform protein inference to handle shared peptides
3. Train ML model on sampled data (if enabled)
4. Create protein groups and apply ML scoring in single pass per file
5. Update global scores and filter in final single pass
"""
...
"""
    estimate_total_protein_groups(psm_paths, protein_inference_dict, precursors, min_peptides)

Estimate total protein groups by sampling first few files.
"""
...
"""
    add_protein_group_features!(df::DataFrame, protein_to_possible_peptides::Dict)

Add protein group features including n_possible_peptides and peptide_coverage.
"""
