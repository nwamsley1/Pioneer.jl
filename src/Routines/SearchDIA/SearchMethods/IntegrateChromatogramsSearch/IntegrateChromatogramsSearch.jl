...
function process_file!(
    ...
    elseif params.match_between_runs && isfile(passing_psms_path * PASS1_SIDECAR_SUFFIX)
        add_mbr_integrated_spectra_to_psms!(
            passing_psms,
            chromatograms,
            getRtIrtModel(search_context, ms_file_idx),
        ...
        _filter_mbr_sidecars_by_rows!(passing_psms_path, Int[])
        passing_psms = passing_psms[Int[], :]
    end
...
function process_search_results!(
    ...
    if nrow(passing_psms) == 0 || ncol(passing_psms) == 0
        ...
        if params.match_between_runs && !isempty(output_path) &&
...
function summarize_results!(
    ...
    if params.match_between_runs &&
       any(path -> isfile(path * PASS1_SIDECAR_SUFFIX), passing_paths)
        ...
        @debug_l1 "Post-integration MBR combined error: baseline=$(summary.base_decoys)/$(summary.base_targets) " *
        ...
        @debug_l1 "Post-integration MBR internal FTR for final accepted target transfers: " *
