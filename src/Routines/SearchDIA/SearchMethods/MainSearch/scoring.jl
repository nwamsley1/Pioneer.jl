...
function train_lgbm_and_select_best(
    ...
    # Pre-LGBM per-precursor aggregates (irt_fwhm, smoothness, max_weight, etc.)
    # broadcast to every training row so the per-file LGBM sees them. cv_fold is
    # precursor-stratified so the broadcast is consistent within each fold (no
    # train/validation leakage).
    if nrow(psms) > 0
        compute_per_precursor_aggregates_broadcast!(psms)
    end
...
"""
    compute_per_precursor_aggregates_broadcast!(psms::DataFrame) -> Nothing

Compute per-precursor aggregate features and broadcast them to EVERY row of
the precursor (so the per-file LightGBM, which trains on per-scan rows,
can use them as features). Operates in-place.

Adds columns:
- `:irt_fwhm`, `:n_above_hm`, `:rt_fwhm`        — chromatographic FWHM stats
- `:smoothness`                                  — Σ((Δ²w/w_apex)²) over scans sorted by RT
- `:num_scans`                                   — number of scans for this precursor
- `:max_weight`, `:max_gof`, `:max_fitted_manhattan_distance` — best-scan summary stats

`:n_scans` (UInt32 row count, broadcast from train_lgbm_and_select_best) is
the same as `:num_scans` (UInt16) — kept separate for back-compat with
existing feature lists / ScoringSearch consumers.

Skips groups where the relevant per-scan column is absent. Idempotent:
existing aggregate columns are overwritten.
"""
function compute_per_precursor_aggregates_broadcast!(psms::DataFrame)
    n = nrow(psms)
    n == 0 && return nothing
    ...
    compute_rt   = has_rt
    compute_smooth = compute_fwhm && compute_rt
    compute_gof  = gof_vec !== nothing
    compute_fmd  = fmd_vec !== nothing

    # Per-row output buffers (broadcasted aggregates).
    irt_fwhm_out   = compute_fwhm  ? Vector{Float32}(undef, n) : nothing
    n_above_hm_out = compute_fwhm  ? Vector{UInt16}(undef, n)  : nothing
    rt_fwhm_out    = (compute_fwhm && compute_rt) ? Vector{Float32}(undef, n) : nothing
    smoothness_out = compute_smooth ? Vector{Float32}(undef, n) : nothing
    num_scans_out  = Vector{UInt16}(undef, n)
    max_weight_out = compute_fwhm  ? Vector{Float32}(undef, n) : nothing
    max_gof_out    = compute_gof   ? Vector{eltype(gof_vec)}(undef, n) : nothing
    max_fmd_out    = compute_fmd   ? Vector{eltype(fmd_vec)}(undef, n) : nothing

    perm = sortperm(prec_ids)

    # Reusable buffers for smoothness
    smooth_w_buf  = Vector{Float32}(undef, 128)
    smooth_rt_buf = Vector{Float32}(undef, 128)
    smooth_ord_buf = Vector{Int}(undef, 128)
    ...
    while gi <= n
        ...
        # Sub-pass 1: max_weight / max_gof / max_fmd
        mw = 0f0
        max_gof_val = compute_gof ? typemin(eltype(gof_vec)) : nothing
        max_fmd_val = compute_fmd ? typemin(eltype(fmd_vec)) : nothing
        ...
        if compute_fwhm
            half_max = 0.5f0 * mw
            irt_lo = typemax(Float32); irt_hi = typemin(Float32)
            rt_lo  = typemax(Float32); rt_hi  = typemin(Float32)
            @inbounds for k in 0:(group_len - 1)
                row = perm[group_start + k]
                weights[row] >= half_max || continue
                n_hm += UInt8(1)
                irt = irt_obs[row]
                irt_lo = min(irt_lo, irt); irt_hi = max(irt_hi, irt)
                if compute_rt
                    r = rt_vals[row]
                    rt_lo = min(rt_lo, r); rt_hi = max(rt_hi, r)
                end
            end
            if n_hm > 0
                irt_fw = irt_hi - irt_lo
                rt_fw  = compute_rt ? rt_hi - rt_lo : 0f0
            end
        end

        # Sub-pass 3: smoothness
        rough = 0f0
        if compute_smooth
        ...
        # Broadcast aggregates to every row in the group
        ns = UInt16(group_len)
        @inbounds for k in 0:(group_len - 1)
            row = perm[group_start + k]
            num_scans_out[row] = ns
            if compute_fwhm
                irt_fwhm_out[row]   = irt_fw
                n_above_hm_out[row] = n_hm
                max_weight_out[row] = mw
            end
            if rt_fwhm_out !== nothing
                rt_fwhm_out[row] = rt_fw
            end
            if smoothness_out !== nothing
                smoothness_out[row] = rough
            end
            if compute_gof
                max_gof_out[row] = max_gof_val
            end
            if compute_fmd
                max_fmd_out[row] = max_fmd_val
            end
        end
    end

    psms[!, :num_scans] = num_scans_out
    if compute_fwhm
        psms[!, :irt_fwhm]   = irt_fwhm_out
        psms[!, :n_above_hm] = n_above_hm_out
        psms[!, :max_weight] = max_weight_out
    end
    if rt_fwhm_out !== nothing
        psms[!, :rt_fwhm] = rt_fwhm_out
    end
    if smoothness_out !== nothing
        psms[!, :smoothness] = smoothness_out
    end
    if compute_gof
        psms[!, :max_gof] = max_gof_out
    end
    if compute_fmd
        psms[!, :max_fitted_manhattan_distance] = max_fmd_out
    end
    return nothing
end
