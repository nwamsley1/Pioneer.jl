...
function train_psm_classifier_with_fallback(
    ...
    n_total = nrow(psms)
    if training_mask !== nothing && length(training_mask) != n_total
        throw(ArgumentError("training_mask length must match psms row count"))
    end
    ...
    #   fold_pairs[1] = trained on cv_fold==1, OOF for idx0,
    #                   in-fold for all idx1 rows when requested.
    #   fold_pairs[2] = trained on cv_fold==0, OOF for idx1,
...
function _ensure_mainsearch_n_scans!(psms::DataFrame)
...
function _train_mainsearch_classifier_iteration!(
    psms::DataFrame;
    features::Vector{Symbol} = collect(PRESCORE_FEATURES),
    lgbm_hp = SHARED_LGBM_HP,
)
    t0 = time()
    _ensure_mainsearch_n_scans!(psms)
...
function train_lgbm_and_select_best(
    ...
    result = _train_mainsearch_classifier_iteration!(
        psms;
        features = features,
        lgbm_hp = lgbm_hp,
    )
    return result.best_psms, result.scores, result.timings, result.predictor
end
...
function _mainsearch_selected_trace_fragment_spectral_angle(
    ...
    isotope_traces,
    frag_cols,
    pred_fragment_intensity_provider!,
    pid::UInt32,
    obs_buf::Vector{Float32},
    pred_buf::Vector{Float32},
)
    fill!(obs_buf, 0f0)
    fill!(pred_buf, 0f0)
    selected_trace = isotope_traces[best_row]
...
function _mainsearch_predicted_fragment_rank_intensities!(
    ...
    pid_i = Int(pid)
    1 <= pid_i <= length(prec_mzs) || return nothing
    spline_data = getSplineData(frag_lookup, nce_model, prec_charges[pid_i], Float32(prec_mzs[pid_i]))
...
function select_best_per_precursor!(
    ...
    frag_obs_sum_buf = zeros(Float32, 8)
    frag_pred_buf = zeros(Float32, 8)
...
#==========================================================
Pair competition (post-LGBM, on best_psms)
==========================================================#

"""
    _build_paircomp_lookups(prec_ids, lgbm_probs, n_lib) -> (row_by_pid, score_by_pid)

Build library-pid-keyed lookup vectors for paircomp. `row_by_pid[pid] == 0`
means "this precursor is not in best_psms"; otherwise it holds the 1-based
row index in best_psms. `score_by_pid[pid]` is valid only when
`row_by_pid[pid] != 0`.

Indexing by library pid replaces the previous `Dict{UInt32, Float32}` /
`Dict{UInt32, Int}` pair (~400k Dict-inserts per file). Memory cost is
2 × 4 × n_lib bytes (≈ 50 MB for a 6M-precursor library), released at
function exit.
"""
function _build_paircomp_lookups(prec_ids::Vector{UInt32},
                                  lgbm_probs::AbstractVector{<:Real},
                                  n_lib::Int)
    row_by_pid   = zeros(Int32, n_lib)
    score_by_pid = Vector{Float32}(undef, n_lib)
    @inbounds for i in 1:length(prec_ids)
        pid = Int(prec_ids[i])
        row_by_pid[pid]   = Int32(i)
        score_by_pid[pid] = Float32(lgbm_probs[i])
    end
    return row_by_pid, score_by_pid
end

"""
    _mark_paircomp_losers!(keep_mask, prec_ids, targets, partner_col,
                            row_by_pid, score_by_pid) -> nothing

For each row i in best_psms, if its precursor pid has a partner in the
library AND that partner is also present in best_psms (row_by_pid > 0)
AND `pid < partner` (canonical orientation — visit each pair exactly
once), compare LGBM scores and mark the loser's row in keep_mask = false.
Ties resolve in favor of the target.

Threaded via `parallel_foreach!` — safe because each loser-row is the
unique target of at most one canonical pair, so no two threads write to
the same `keep_mask` index.
"""
function _mark_paircomp_losers!(keep_mask::Vector{Bool},
    ...
    parallel_foreach!(n0) do chunk
        @inbounds for i in chunk
            ...
            ismissing(ptr_raw) && continue
            partner = UInt32(ptr_raw)
            # Canonical orientation: process pair only at the row with the
            # smaller pid. Filters out unpaired (partner==0) and self-pairs.
            ...
            loser_row = if my_score < pt_score
                i
            elseif my_score > pt_score
                partner_row
            else
                # Tie → drop the decoy (target wins).
                targets[i] ? partner_row : i
            end
            keep_mask[loser_row] = false
        end
    end
    return nothing
end
...
function apply_pair_competition!(best_psms::DataFrame, search_context::SearchContext)
    ...
    n_pairs = n_dropped_t + n_dropped_d
    deleteat!(best_psms, .!keep_mask)
    return n_pairs, n_dropped_t, n_dropped_d
end
