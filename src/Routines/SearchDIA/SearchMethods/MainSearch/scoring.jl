...
# Lightweight per-file MainSearch model (config E): smaller trees + coarser bins,
# half the iterations at double the learning rate (same effective boosting).
...
# which keep SHARED_LGBM_HP / SCORING_LGBM_HP. Matches the env-sweep "E" arm.
const MAINSEARCH_LGBM_HP = merge(SHARED_LGBM_HP,
    (num_iterations = 25, learning_rate = 0.40, max_depth = 4, num_leaves = 15, max_bin = 63))
