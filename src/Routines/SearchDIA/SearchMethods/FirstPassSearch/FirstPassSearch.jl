...
function summarize_results!(
    ...
    if false==true#params.match_between_runs == true
        ...
        # Count precursors before MBR (for comparison)
        targets_with_partners = 0
        decoys_with_partners = 0
        targets_missing_partners = 0
        decoys_missing_partners = 0
        partners_added = 0
        partners_already_present = 0
        
        # Track CV fold splits
        total_pairs = 0
        same_fold_pairs = 0
        split_fold_pairs = 0
        ...
        for pid in keys(precursor_dict)
            ...
            if ismissing(partner_pid)
                # Count precursors without partners
                if is_decoy_array[pid]
                    decoys_missing_partners += 1
                else
                    targets_missing_partners += 1
                end
            else
                # Has a partner
                if is_decoy_array[pid]
                    decoys_with_partners += 1
                else
                    targets_with_partners += 1
                end
                
                # Check if both members are in dictionary (after MBR should have added them)
                if haskey(precursor_dict, partner_pid)
                    # Both in dictionary - check CV folds
                    pair_key = pid < partner_pid ? (pid, partner_pid) : (partner_pid, pid)
                    
                    if !(pair_key in checked_pairs)
                        push!(checked_pairs, pair_key)
                        total_pairs += 1
                    ...
                    partners_already_present += 1
                else
                    # Partner should have been added by MBR but wasn't!
                    @user_warn "Partner $partner_pid not in dictionary for precursor $pid"
                    partners_added += 1  # This should have been added
                end
            end
        end
        
        @user_info "MBR Partner Analysis:"
        @user_info "  Targets with partners: $targets_with_partners"
        @user_info "  Targets missing partners: $targets_missing_partners"
        @user_info "  Decoys with partners: $decoys_with_partners"
        @user_info "  Decoys missing partners: $decoys_missing_partners"
        @user_info "  Partners that should be in dict: $(targets_with_partners + decoys_with_partners)"
        @user_info "  Complete pairs found: $total_pairs"
        
        if total_pairs > 0
            split_percentage = round(100.0 * split_fold_pairs / total_pairs, digits=1)
            @user_info ""
            @user_info "CV Fold Assignment for Complete Pairs:"
            @user_info "  Pairs in same fold: $same_fold_pairs"
            @user_info "  Pairs split across folds: $split_fold_pairs"
            @user_info "  Percentage of split pairs: $split_percentage%"
            
            if split_fold_pairs > 0
                @user_info "⚠️ Warning: $split_fold_pairs target-decoy pairs are split across CV folds!"
                @user_info "  This causes data leakage when MBR is enabled."
            else
                @user_info "✓ All target-decoy pairs are in the same CV fold."
            end
        end
    end
    # Count targets and decoys in the final precursor dictionary
    @user_info "FirstPassSearch precursor dictionary composition:"
    precursors = getPrecursors(getSpecLib(search_context))
    is_decoy_array = getIsDecoy(precursors)
    
    target_count = 0
    decoy_count = 0
    total_count = length(precursor_dict)
    ...
    @user_info "  Total precursors in dictionary: $total_count"
    @user_info "  Targets: $target_count"
    @user_info "  Decoys: $decoy_count"
    @user_info "  Target/Decoy ratio: $(round(target_count / max(decoy_count, 1), digits=2))"
