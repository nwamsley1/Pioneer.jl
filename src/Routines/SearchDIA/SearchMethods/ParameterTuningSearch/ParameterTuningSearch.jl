...
# Refactored process_file! function using extracted helper functions
...
# Old process_file! function (keeping for reference - to be removed after testing)
function process_file_OLD!(
    results::ParameterTuningSearchResults,
    params::P, 
    search_context::SearchContext,    
    ms_file_idx::Int64,
    ...
    Collect PSMs and log results with context message.
    Returns tuple of (psms, psm_count).
    """
    ...
    Fit mass error model from PSMs. Returns (mass_err_model, ppm_errs, psm_count).
    Returns (nothing, nothing, psm_count) if no fragments found.
    """
    ...
    Check convergence and store results if converged. Returns true if converged.
    """
    ...
    Add scans using doubling strategy. Returns false if max scans reached.
    """
    ...
    Expand mass tolerance by factor of 2 (capped at max).
    """
    ...
    Execute one of three convergence strategies. Returns (psms, mass_err_model, ppm_errs).
    """
    function execute_strategy(strategy_num::Int, filtered_spectra, spectra, search_context, 
        ...
        elseif strategy_num == 3
            ...
            error("Invalid strategy number: $strategy_num")
        end
    ...
    function handle_error!(results, search_context, ms_file_idx, e, parsed_fname)
        ...
        status = ParameterTuningStatus(
            ...
            false,  # not converged
            true,   # used fallback
            "Error during tuning: $(typeof(e))",
            0,      # no attempts completed
            0,      # no PSMs
