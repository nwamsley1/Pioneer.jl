...
function quadTuningSearch(    rt_to_irt_map_dict,
    ...
    for (ms_file_idx, MS_TABLE_PATH) in ProgressBar(collect(enumerate(MS_TABLE_PATHS)))
        try
            ...
            ##########
            #Check for multiple window widths (not currently supported)
            window_widths = Set{String}()
            for i in range(1, length(MS_TABLE[:isolationWidthMz]))
                if MS_TABLE[:msOrder][i] == 2
                    if string(MS_TABLE[:isolationWidthMz][i]) ∉ window_widths
                        push!(window_widths, string(MS_TABLE[:isolationWidthMz][i]))
                    end
                end
            end
            if length(window_widths) != 1
                window_widths = join(collect(window_widths), ';')
                @warn "Resorting to default quad transmission function. Does not support multiple window sizes in a single raw file at this time.
