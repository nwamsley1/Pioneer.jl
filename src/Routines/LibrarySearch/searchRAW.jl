function SearchRAW(
    ...
    n = 0
    start = 1
    println("fragment_tolerane ", fragment_tolerance)
    ...
    tasks = map(ranges) do _range
        ...
        Threads.@spawn begin 
            ...
            for i in range(first(_range), last(_range))
                ...
                (i >= first(_range)) & (i <= last(_range)) ? nothing : continue #Skip if outside the scan range
