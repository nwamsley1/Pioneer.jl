...
function selectTransitions(fragment_list::Vector{Vector{LibraryFragment{V}}}, rt_index::retentionTimeIndex{T, U}, rt::T, rt_tol::T, prec_mz::U, prec_tol::U) where {T,U,V<:AbstractFloat}
    ...
    function addTransitions!(transitions::Vector{LibraryFragment{V}}, prec_ids::Vector{UInt32}, fragment_list::Vector{Vector{LibraryFragment{V}}}, precs::Vector{Tuple{UInt32, U}}, prec_mz::U, prec_tol::U)
    ...
    for i in rt_start:rt_stop #Add transitions
        addTransitions!(transitions, prec_ids, fragment_list, rt_index.rt_bins[i].prec, prec_mz, prec_tol)
    end

    return sort!(transitions, by = x->getFragMZ(x)), prec_ids #Sort transitions by their fragment m/z. 
end
