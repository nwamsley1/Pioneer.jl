...
function makeRTIndices(psms_dict::Dictionary{String, DataFrame}, 
    ...
    for (file_path, psms) in ProgressBar(pairs(psms_dict))
        ...
        #i = 1
        for (i, (prec_id, value)) in ProgressBar(enumerate(pairs(precID_to_iRT)))
            prec_ids[i] = prec_id
            if prec_id ∈ prec_set::Dictionary{UInt32, Tuple{Float32, Float32}}
                rt, mz = prec_set[prec_id]::Tuple{Float64, Float32}
                RTs[i] = rt
                mzs[i] = mz
            else
...
@profview makeRTIndices(PSMs_Dict,precID_to_iRT,iRT_RT)
Dictionary(PSMs[!,:precursor_idx], PSMs[!,:])

PSMs_test = PSMs_Dict.values[1]


best_hits = (PSMs_test[!,:spectral_contrast].>0.9) .& (PSMs_test[!,:decoy].==false) .& (PSMs_test[!,:entropy_score].>0.9) .& (PSMs_test[!,:total_ions].>6)
linear_spline = KDEmapping(PSMs_test[best_hits,:iRT],
PSMs_test[best_hits,:RT]
)
