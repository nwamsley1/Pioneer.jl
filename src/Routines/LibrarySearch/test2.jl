mutable struct Counter{I,C<:Unsigned,T<:AbstractFloat}
    ids::Vector{I}
    counts::Vector{Tuple{C, T, T, T}}
    size::Int64
    matches::Int64
    function Counter(I::DataType, C::DataType, T::DataType, size::Int) #where {I,C<:Unsigned}
        new{I, C, T}(zeros(I, size), [(zero(C), zero(T), zero(T), zero(T)) for x in 1:size], 1, 0)
    end
end
...
function update!(c::Counter{I,C,T}, id::I, pred_intensity::T, pred::T, obs::T) where {C,I<:Unsigned,T<:AbstractFloat}
    #count = c.counts[id]
    #c.counts[id] = (first(count) + one(C), last(count) + pred_intensity);
    c.counts[id] = (c.counts[id][1] + one(C), c.counts[id][2] + pred_intensity, c.counts[id][3] + pred, c.counts[id][4] + obs);
    return nothing
end

function reset!(c::Counter{I,C,T}, id::I) where {C,I<:Unsigned,T<:AbstractFloat}
    c.counts[id] = (zero(C), zero(T), zero(T), zero(T));
    return nothing
end
...
function inc!(c::Counter{I,C,T}, id::I, pred_intensity::T, pred::T, obs::T) where {I,C<:Unsigned,T<:AbstractFloat} 
    if iszero(first(c.counts[id]))#c.counts[id]<1#iszero(c.counts[id])# == zero(C)
        c.ids[getSize(c)] = id;
        update!(c,id,pred_intensity, pred, obs);
        incSize!(c);
    else
        update!(c,id,pred_intensity, pred, obs);
    end
...
function reset!(c::Counter{I,C,T}) where {I,C<:Unsigned,T<:AbstractFloat} 
    ...
    for i in 1:(getSize(c) - 1)
        c.counts[c.ids[i]] = (zero(UInt8), zero(Float32), zero(Float32), zero(Float32));
    end
    c.size = 1
    c.matches = 0
    return nothing
...
frag_matches = [(id, int, randn(Float32), randn(Float32)) for (id, int) in zip(prec_ids, prec_intensity)]
prosit_totals = [randn(Float32) for x in 1:9387261]
function benchmark(precs::Counter{I,C,T}, frag_matches::Vector{Tuple{UInt32, Float32, Float32, Float32}}, prosit_totals::Vector{Float32}) where {I,C<:Unsigned,T<:AbstractFloat} 
    for match in frag_matches
        inc!(precs, match[1], match[2], match[3], match[4]);
    end
