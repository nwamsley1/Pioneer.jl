...
function searchPrecursorBin!(precs::Dict{UInt32, UInt8}, precursor_bin::PrecursorBin{T}, window_min::Float64, window_max::Float64) where {T<:AbstractFloat}
    ...
    function addFragmentMatches!(precs::Dict{UInt32, UInt8}, precursor_bin::PrecursorBin{T}, window_min::AbstractFloat, window_max::AbstractFloat, start::Int, stop::Int) where {T<:AbstractFloat}
...
function searchScan!(precs::Dict{UInt32, UInt8}, f_index::FragmentIndex{T}, massess::Vector{Union{Missing, U}}, intensities::Vector{Union{Missing, U}}, precursor_window::AbstractFloat, ppm::AbstractFloat, width::AbstractFloat; topN::Int = 20, min_frag_count::Int = 3) where {T,U<:AbstractFloat}
    
    getFragTol(mass::U, ppm::AbstractFloat) = mass*(1 - ppm/1e6), mass*(1 + ppm/1e6)

    function filterPrecursorMatches!(precs::Dict{UInt32, UInt8}, topN::Int, min_frag_count::Int) where {T<:AbstractFloat}
        #Do not consider peptides wither fewer than 
        match_count = sum(values(precs))
        prec_count = length(keys(precs))
        #return [first(x) for x in sort(filter(x->last(x)>=min_frag_count,pairs(precs)), by=x->last(x), rev = true)][1:min(topN, end)], match_count, prec_count
        return (first(pair) for pair in sort(filter(pair -> last(pair) > 1, pairs(test_acc)), by = pair -> last(pair), rev = true)), match_count, prec_count
    end
    #println("TEST")
    min_frag_bin = 0
    #println("start")
