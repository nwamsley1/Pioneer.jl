...
function getSub(mean::Float32, array::AbstractArray, lower_bound::Float32, upper_bound::Float32)
...
for (key, value) in pairs(getIDToLightHeavyPair(test))
    ...
    append!(getIDToLightHeavyPair(test)[key].heavy_scan_idxs,
    getSub(test.prec_id_to_precursor[value.heavy_prec_id], GAPDH_VGVNGFGR[:precursorMZ]))
end
