...
Fill a pre-allocated vector with all possible combinations of variable modifications as PeptideMod vectors.

# Arguments
- `all_mods::Vector{Vector{PeptideMod}}`: Pre-allocated vector to store modification combinations
- `var_mod_matches::Vector{NamedTuple{(:regex_match, :name), Tuple{RegexMatch, String}}}`: Vector of modification matches from matchVarMods()
- `fixed_mods::Vector{PeptideMod}`: Fixed modifications to include in all combinations
- `max_var_mods::Int`: Maximum number of variable modifications allowed per sequence

# Effects
Modifies `all_mods` in place, filling it with all valid modification combinations.

# Notes  
Each element of `all_mods` contains both fixed modifications (always included) and
a specific combination of variable modifications. The function generates combinations
systematically using combinatorial enumeration.
"""
function fillVarModStrings!(
    ...
    for n_mods in 0:min(max_var_mods, n_var_mods)
        for combo in combinations(1:n_var_mods, n_mods)
            all_mods[i] = copy(fixed_mods)
            for mod_idx in combo
                var_match = var_mod_matches[mod_idx]
                push!(all_mods[i], PeptideMod(
                    UInt8(var_match.regex_match.offset),
                    first(var_match.regex_match.match),
                    var_match.name
                ))
            end
            i += 1
