...
function add_charge_specific_partner_columns!(df::DataFrame)
    """
    Creates target-decoy pairs using (base_pep_id, precursor_charge) combination.
    Each base_pep_id/precursor_charge combo should have exactly one target and exactly one decoy.
    Uses combination of base_pep_id and precursor_charge to assign unique pair_ids.
    """
    result_df = copy(df)
    n = nrow(df)
    ...
    decoys = sum(df.decoy)
    @user_info "Starting pairing by (base_pep_id, precursor_charge): $targets targets, $decoys decoys"
    
    # Create lookup dictionary: (base_pep_id, precursor_charge, is_target) -> row_index
    ...
    for idx in 1:n
        ...
        # Look for decoy partner with same base_pep_id and precursor_charge
        ...
        if haskey(lookup, decoy_key)
            ...
            @debug_l2 "No decoy found for target: base_pep_id=$(row.base_pep_id), precursor_charge=$(row.precursor_charge)"
