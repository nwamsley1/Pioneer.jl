...
getSubBinRange(fb::FragmentIndexBin{T}) where {T<:AbstractFloat} = fb.br
...
FragIndexBin() = FragIndexBin(0.0, 0.0, UnitRange{UInt32}(0, 0))
...
struct FragmentIndex{T<:AbstractFloat}
    ...
    precursor_bins::Vector{IndexFragment{T}}
end
...
getFragments(fi::FragmentIndex{T}) where {T<:AbstractFloat} = fi.precursor_bins
