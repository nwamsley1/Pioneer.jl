...
# C13_C12_MASS_DIFF is already imported into Main via test_setup.jl. Re-`const`
# declaring it here is fatal under Julia 1.12 (was a warning under 1.11).
...
@testset "fused per-precursor filters" begin
    ...
    @testset "quad_window_with_iso_bounds" begin
        ...
        @test low  === 500.0f0 - Float32(C13_C12_MASS_DIFF * (-1) / 2)
        @test high === 510.0f0 + Float32(C13_C12_MASS_DIFF *  1  / 2)

        # Charge dependence: |C13_C12_MASS_DIFF/charge| is smaller at higher charge,
        ...
        for charge in (1, 2, 3, 4)
            ...
            @test l === 500.0f0 - Float32(C13_C12_MASS_DIFF * (-1) / charge)
            @test h === 510.0f0 + Float32(C13_C12_MASS_DIFF *   1  / charge)
        end

        # Asymmetric bounds (-2, 0) → first/last applied independently.
        low_a, high_a = quad_window_with_iso_bounds(qfunc, 2, (-2, 0))
        @test low_a  === 500.0f0 - Float32(C13_C12_MASS_DIFF * (-2) / 2)
        @test high_a === 510.0f0 + Float32(C13_C12_MASS_DIFF *   0  / 2)
    end
    ...
    @testset "bit-identical to inline original" begin
        ...
        for prec_charge in (1, 2, 3, 4), bounds in ((-1,1), (-2,0), (0,2), (-3,3))
            ref_lo = Pioneer.getPrecMinBound(qfunc) -
                     Float32(C13_C12_MASS_DIFF * first(bounds) / prec_charge)
            ref_hi = Pioneer.getPrecMaxBound(qfunc) +
                     Float32(C13_C12_MASS_DIFF * last(bounds)  / prec_charge)
