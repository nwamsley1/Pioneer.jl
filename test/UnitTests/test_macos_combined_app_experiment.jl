...
@testset "macOS combined app PackageCompiler experiment" begin
    ...
    @test occursin("create_app(\n              \"packages/PioneerPredict\"", macos_workflow)
    @test occursin("create_app(\n              \"packages/PioneerSearch\"", macos_workflow)
    @test !occursin("create_app(\n              \"packages/PioneerParams\"", macos_workflow)
    @test occursin("apps/search/bin/\$executable_name", unix_launcher)
    @test occursin("apps/predict/bin/\$executable_name", unix_launcher)
    @test occursin("apps/runtime/bin/\$executable_name", unix_launcher)
    @test findfirst("apps/search/bin/\$executable_name", unix_launcher) <
          findfirst("apps/runtime/bin/\$executable_name", unix_launcher)
    @test findfirst("apps/predict/bin/\$executable_name", unix_launcher) <
          findfirst("apps/runtime/bin/\$executable_name", unix_launcher)
    @test occursin("subapp_root=\"\$SCRIPT_DIR/apps/search\"", unix_launcher)
    @test occursin("subapp_root=\"\$SCRIPT_DIR/apps/predict\"", unix_launcher)
    @test occursin("subapp_root=\"\$SCRIPT_DIR/apps/runtime\"", unix_launcher)
end
