...
@testset "PioneerPlots package boundary" begin
    ...
    plots_qc = joinpath(REPO_ROOT, "packages", "PioneerPlots", "src", "routines", "qc_plots.jl")
    plots_rt_alignment = joinpath(REPO_ROOT, "packages", "PioneerPlots", "src", "routines", "plot_rt_alignment.jl")
    ...
    plots_module_content = read(plots_module, String)
    plots_qc_content = read(plots_qc, String)
    common_module_content = read(common_module, String)

    @test !occursin("src\", \"structs\", \"MassSpecData.jl", plots_module_content)
    @test !occursin("src\", \"structs\", \"MassErrorModel.jl", plots_module_content)
    @test !occursin("src\", \"utils\", \"safeFileOps.jl", plots_module_content)
    @test !occursin("BasicMassSpecData", plots_qc_content)
    @test occursin("_read_tic_series", plots_qc_content)
    @test occursin("export safeRm", common_module_content)
    ...
    for plot_file in (plots_qc, root_qc, plots_rt_alignment)
        ...
        @test !occursin("Dict{Int64, MassErrorModel}", join(active_lines, '\n'))
        @test !occursin("getMassCorrection(frag_err_dist_dict", join(active_lines, '\n'))
        @test !occursin("Dictionary{String, Any}", join(active_lines, '\n'))
    end
    ...
    @test occursin("export RtAlignmentPlotSpec", common_module_content)
    @test occursin("export MassErrorPlotSpec", common_module_content)
    ...
    @test !occursin("\nusing LaTeXStrings\n", search_content)
    @test occursin("const Plots = PioneerPlots.Plots", search_content)
