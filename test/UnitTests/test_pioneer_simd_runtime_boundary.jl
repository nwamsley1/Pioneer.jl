...
@testset "PioneerSIMD owns runtime-only SIMD acceleration" begin
    ...
    search_entrypoint = read(joinpath(REPO_ROOT, "packages", "PioneerSearch", "src", "routines", "SearchDIA.jl"), String)
    ...
    @test occursin(r"(?s)create_app\(\s*\"packages/PioneerSearch\".*?include_transitive_dependencies=false", linux_workflow)
    @test occursin(r"(?s)create_app\(\s*\"packages/PioneerPredict\".*?include_transitive_dependencies=false", windows_workflow)
    @test occursin(r"(?s)create_app\(\s*\"packages/PioneerSearch\".*?include_transitive_dependencies=false", windows_workflow)
    @test occursin(r"(?s)create_app\(\s*\"packages/PioneerPredict\".*?include_transitive_dependencies=false", macos_workflow)
    @test occursin(r"(?s)create_app\(\s*\"packages/PioneerSearch\".*?include_transitive_dependencies=false", macos_workflow)

    @test !isfile(joinpath(REPO_ROOT, "packages", "PioneerPredict", "LocalPreferences.toml"))
    @test !isfile(joinpath(REPO_ROOT, "packages", "PioneerSearch", "LocalPreferences.toml"))
end
