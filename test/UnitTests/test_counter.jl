...
@testset "Counter or! (bitmask)" begin
    ...
    @testset "or! performance parity" begin
        n_ids = 1000
        n_ops = 100_000
        c_or = Pioneer.Counter(UInt16, UInt8, n_ids + 1)
        c_inc = Pioneer.Counter(UInt16, UInt8, n_ids + 1)

        # Warmup
        for _ in 1:1000
            Pioneer.or!(c_or, UInt16(1), UInt8(0x01))
            Pioneer.inc!(c_inc, UInt16(1), UInt8(0x01))
        end
