...
lightMZ = getMZ(Precursor(getResidues("VGVNGFGR"), Int32(2)))
heavyMZ = getMZ(Precursor(getResidues("VGVNGFGR[+10.008269]"), Int32(2)))
...
function getIntegrationBounds(indices::Vector{Int64}; max_gap_size::Int = 10)
    start = 1
    stop = 2
    last = indices[start]
    best_start = 1
    best_stop = 2
    gap_size = 0
    for index in enumerate(diff(indices))
        println(gap_size)
        #println("index[2] ", index[2])
        #println("last ", last)
        gap_size = indices[index[1]] - last
...
function getHits(MzFeatures::Vector{NamedTuple{(:low, :mass, :high), Tuple{Float64, Float32, Float64}}}, 
...
function getMzFeatures(mass_list::Vector{Float32}, ppm::Float32)
     (map(x->(low=x-(x/1000000.0)*ppm, mass = x, high=x+(x/1000000.0)*ppm), mass_list))
end
