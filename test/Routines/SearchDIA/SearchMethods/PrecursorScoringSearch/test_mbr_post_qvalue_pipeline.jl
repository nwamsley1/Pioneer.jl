...
@testset "MBR counterfactuals require same-charge same-length precursors" begin
    mktempdir() do dir
        ...
        other_df = DataFrame(
            ...
            scan_idx = UInt32[201, 203],
            weight = Float32[10, 3],
            log2_intensity_explained = Float16[1, 0.5],
            irt_pred = Float32[10.0, 10.20],
            irt_obs = Float32[10.1, 10.1],
            log_by_ratio_m0 = zeros(Float16, 2),
            rt = Float32[5.0, 5.2],
            n_scans = Float32[8, 5],
            main_pep = Float32[0.001, 0.001],
            smoothed_2d_shadow_hellinger = Float32[0.1, 0.3],
            ms_file_idx = fill(UInt32(2), 2),
            target = trues(2),
            cv_fold = zeros(UInt8, 2),
            trace_prob = Float32[0.80, 0.70],
            trace_prob_prepass = Float32[0.80, 0.70],
            trace_prob_infold = Float32[0.80, 0.70],
            prec_prob = Float32[0.80, 0.70],
            qval = Float32[0.001, 0.001],
            pep = Float32[0.001, 0.001],
            global_qval = Float32[0.001, 0.001],
            global_pep = Float32[0.001, 0.001],
            mbr_recovered = falses(2),
            frag1_smoothed_intensity = Float32[100, 60],
            frag2_smoothed_intensity = Float32[0, 40],
            frag3_smoothed_intensity = zeros(Float32, 2),
            frag4_smoothed_intensity = zeros(Float32, 2),
            frag5_smoothed_intensity = zeros(Float32, 2),
            frag6_smoothed_intensity = zeros(Float32, 2),
...
@testset "MBR counterfactuals choose closest library iRT within same length and charge" begin
    mktempdir() do dir
        ...
        other_df = DataFrame(
            ...
            log2_intensity_explained = Float16[1, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5],
...
@testset "post-integration counterfactual pairing uses refined library iRT" begin
    mktempdir() do dir
        ...
        pool = _mbr_post_q_candidate_table(ms_file_idx = 2, scan_offset = 2000, passing = true)[1:3, :]
        ...
        pool[!, :irt_pred] = Float32[10.0, 10.01, 50.0]
        pool[!, :MBR_integrated_apex_irt_obs] = Float32[49.0, 10.0, 51.0]
        pool[!, :trace_prob] = Float32[0.90, 0.60, 0.80]
        pool[!, :trace_prob_prepass] = Float32[0.90, 0.60, 0.80]
        pool[!, :trace_prob_infold] = Float32[0.90, 0.60, 0.80]
        pool[!, :prec_prob] = Float32[0.90, 0.60, 0.80]
        pool[!, :MBR_integrated_weight] = Float32[10.0, 20.0, 30.0]
        pool[!, :MBR_integrated_log2_intensity_explained] = Float32[1.0, 2.0, 3.0]
