...
the OOF scores it aggregates: build the run grouping \textbf{separately for each precursor-keyed CV
fold}, from that fold's presence. Empirically this is stable and free:
\begin{center}\small
\begin{tabular}{lccc}
\toprule
dataset & per-fold homogeneity & ARI(fold0, fold1) & note \\
\midrule
EWZ  & $1.00$ (both folds) & $0.684$ & sub-partition of the 20-run conditions differs, both pure \\
APMS & $0.30$ (both folds) & $0.907$ & folds agree strongly on run structure \\
\bottomrule
\end{tabular}
\end{center}
Per-fold groupings are as good as the global one, and the folds \emph{agree} on the condition-level
structure (APMS ARI $0.91$; EWZ $0.68$ only because the arbitrary within-condition split point
differs between folds --- both remain $1.00$ homogeneous, so the disagreement is harmless). This
confirms per-fold clustering is the proper, OOF-consistent choice at no cost in quality.

\section{Recommended recipe}
\begin{enumerate}[nosep]
  \item \textbf{Per CV fold}, from that fold's data:
  \item Presence at $q<10^{-3}$ (sparse CSR = \code{file\_pid}).
