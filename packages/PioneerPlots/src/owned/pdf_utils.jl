...
function create_multipage_pdf(plots::AbstractVector, dest::String)
    withenv("GKSwstype" => "100", "GKS_WSTYPE" => "100") do
        Plots.gr()
        PIONEER_PLOTS_GR.beginprint(dest)
        try
...
function save_multipage_pdf(plots::AbstractVector{<:Plots.Plot}, dest::String)
    ...
    if !isempty(parent_dir) && parent_dir != "."
        mkpath(parent_dir)
    end
    create_multipage_pdf(plots, dest)
    return dest
end
...
function save_multipage_pdf(plot_specs::AbstractVector{<:AbstractPioneerPlotSpec}, dest::String)
    ...
    create_multipage_pdf(map(render_plot, plot_specs), dest)
    return dest
end
